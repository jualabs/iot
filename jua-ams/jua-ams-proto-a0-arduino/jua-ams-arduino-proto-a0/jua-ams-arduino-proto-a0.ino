...
void lerMPU() {
  ...
  Serial.print(F(" | gyr_x = ")); Serial.println(acc_samples[cur_sample].GyX);
  // envia valor 'y' do giroscopio para a serial
  Serial.print(F(" | gyr_y = ")); Serial.println(acc_samples[cur_sample].GyY);
...
void writeAccData() {
  ...
  for(uint16_t i = 0; i < cur_sample; i++) {
    ...
    dadosAcc.print(F(";")); dadosAcc.println(acc_samples[i].GyX);
    // escreve valor 'y' do giroscopio no arquivo
    dadosAcc.print(F(";")); dadosAcc.println(acc_samples[i].GyY);
